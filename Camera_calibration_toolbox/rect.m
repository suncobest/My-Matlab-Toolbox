function [Irec] = rect(I,R,f,c,k,alpha)
if nargin < 6,
    alpha = 0;
    if nargin < 5,
        k = [0;0;0;0;0];
...
ind_lu = px0 * nr + py0 + 1;
ind_ru = (px0 + 1) * nr + py0 + 1;
ind_ld = px0 * nr + (py0 + 1) + 1;
ind_rd = (px0 + 1) * nr + (py0 + 1) + 1;

ind_new = (px(good_points)-1)*nr + py(good_points);
Irec(ind_new) = clu .* I(ind_lu) + cru .* I(ind_ru) + cld .* I(ind_ld) + crd .* I(ind_rd);
