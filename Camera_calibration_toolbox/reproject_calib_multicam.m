...
if ~exist('imsize','var'),
    fprintf(1,'WARNING: No image size available. Setting ''imsize=[640;480]*ones(1,n_cam);''\n');
    imsize=[640;480]*ones(1,n_cam);
end;
...
if isempty(ind_active_views),
    fprintf(1,'No image of camera 1 is available! Please check your data!\n');
    return;
end;

flag = length(ind_active_views)*n_cam>30;
no_grid = 0;
    ...
    for pp = 1:n_cam,
        if active_view(pp),
            ...
            if isempty(x_kk) || isempty(y_kk),
                fprintf(1,'\nWARNING: Data of (camera %d, frame %d) not available!\n',pp,kk);
                continue;
            end;
            %%% load image
            nx = imsize(1,pp);
            ...
            figure(6);
            image(I); axis image;
            title(['(camera ' num2str(pp) ', image ' num2str(kk) ') - Image points (+) and reprojected grid points (o)']);
            colormap(gray(256));
            hold on;
