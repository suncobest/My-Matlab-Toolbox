function [XL,XR] = stereo_triangulation2(xL,xR,om,T,hand,fcl,ccl,kcl,alpha_l,fcr,ccr,kcr,alpha_r)

% [XL,XR] = stereo_triangulation2(xL,xR,om,T,hand,fcl,ccl,kcl,alpha_l,fcr,ccr,kcr,alpha_r),
%
...
%
% Note: XR and XL are related to each other through the rigid motion equation: XR = R * diag([1 1 hand]) * XL + T, where R = rodrigues(om)
...
u = R * diag([1 1 hand]) * xnl;  % 对矢量xnl进行旋转(反射)，将坐标系CL中的矢量转变为坐标系CR中的矢量。
...
X2 = diag([1 1 hand])* R'*(xnr.*repmat(Zr,[3,1])  - T_vect);  % 原理上X1=X2（若非对应点，则不相等）
