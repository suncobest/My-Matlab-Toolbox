function [Xmat, goodpts] = compute_structure2(xbody,omcmat,Tmat,handvec,fmat,cmat,kmat,alphavec,MaxIter)
% compute_structure2
%
...
for pp = 1:n_cam,
    xndata(:,:,pp) = project_points_mirror2(Xactive,omcmat(:,pp),Tmat(:,pp),handvec(pp), ...
...
%     fprintf(1,'%d...',iter);
    if updateJ,
        ...
        for pp = 1:n_cam,
            [xn,~,~,~,~,~,~,dxdX] = project_points_mirror2(Xactive,omcmat(:,pp),Tmat(:,pp), ...
...
if onecam,
    ...
    for i = 1:ncam,
        x = project_points_mirror2(X,om(:,i),T(:,i),hand(i),f,c,k,alpha);
        if checkoutpic,
...
[X1,goodpts] = compute_structure2(xxx,om,T,hand,f,c,k,alpha);
[X2,goodpts2] = compute_structure2(xxx,om,T,hand,f,c,k,alpha,20);   % after iteration
fprintf(1,'\n');
if checkoutpic,
    indx = all(~isnan(X1),1);
    err = X1(:,indx)-X(:,indx);
