...
if ~quest,      % rect all the images
    ...
    for pp = 1:n_cam,
        ...
        for kk = 1:n_ima,
            ima_name = [calib_name strnum_cell{pp}{kk} '.' format_image];
    ...
    fprintf(1,'\n');
     if n_cam==1,
        ...
        pp = 0;
        while ~pp,
            pp = input(['Please input the camera number of the image (from 1:' num2str(n_cam) ', []=1)>>']);
            if isempty(pp),
                pp = 1;
            elseif pp>n_cam || pp<1,
                pp = 0;
                fprintf(1,'\nUnexpected input for camera number, please input again!\n');
            end;
        end;
    end;
    fc = fc_mat(:,pp);
    ...
    assert ((m==ny)&&(n==nx),'Image size do not match with the chosen camera!');
