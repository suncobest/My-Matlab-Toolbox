        ...
        if exist(ima_name,'file')==2,
            ...
            while (~grid_success),
                ...
                if active_view,
                    ...
                    % 若theta = atan2(y_v,x_v)对应的幅角为(a1,a2,a3,a4), 得到角度差theta-theta(1)为
                    % (a1-a1,a2-a1,a3-a1,a4-a1)=angle,则theta = atan2(-y_v,x_v)的幅角度为
                    % (-a1,-a2,-a3,-a4)，角度差theta-theta(1)为(a1-a1,a1-a2,a1-a3,a1-a4)=-angle. 在
                    % 图像坐标系中，幅角的正方向(从x轴旋转到矢量)为顺时针angle，法线朝里；但棋盘图案
                    % 中世界坐标系的xy平面在图像上的投影逆时针为正(-angle)，法线朝外。
                    % mod(x,y)=x-n.*y 其中 n=floor(x./y)。由于角度差a满足-2*pi<=a<=2*pi，若a>=0,则
                    % mod(a,2*pi)=a；若a<0,则mod(a,2*pi)=2*pi+a。于是0<=mod(a,2*pi)<2*pi，将a调整到
                    % [0,2*pi)。对-angle进行排序即是从原点逆时针排序,angle则反之。因此以-angle排序得
                    % 到逆时针为正向的右手系(handedness=1)；以angle排序得到顺时针为正向的左手系(hand=-1)。
