function [xc,good,bad,type] = cornerfinder3(xt,I,wintx,winty,wx2,wy2)

%[xc] = cornerfinder3(xt,I);
%
...
for i=1:N,
    ...
    while (norm(v_extra) > resolution) & (compt<MaxIter),
        ...
        % 结果xc2=[0,0]/0=[NaN,NaN]，此时无角点，警告并返回坐标原点[0 0]
        if (isnan(xc2(1)) || isnan(xc2(2))),
            % warning('The neighborhood of this point have uniform intensity, thus it cann''t be optimized!');
            xc2 = [0 0];
        end
...
% For the diverged points, keep the original guesses: 品质差的点，保留为初始值
