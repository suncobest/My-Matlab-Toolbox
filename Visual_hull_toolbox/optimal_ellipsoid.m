function [cc_out, ax_out, vec_out, ind, ratio] = optimal_ellipsoid(XX, cc_in, ax_in, vec_in, MaxIter)
% Compute the optimal fitting ellipsoid which have same volumn as initial
...
%
% Output:
%   cc_out: center of points cloud in the optimal ellipsoid;
%   ax_out: semi axes of optimal ellipsoid in descending order of length;
%   vec_out: orientation of principal axes correspoinding to semi axes (unit vectors);
%   ind: column logical indices of points in the optimal ellipsoid.
%   ratio: ratio of optimal ellipsoid's axis to the principal axis of points inside.
...
%  See also principal_ellipsoid, ellipsoid_fitting.
...
% choose the closest axes orientation as initial axes
idx = 1:m;
vec = vec_in;
for i = 1:m,
    vc = vec_out(:,i);
    ...
    vec(:,YY) = NaN;   % delete the vector but keep seat
end;
vec_out = vec_out(:,idx);
ax_out = ax_out(idx);
...
if m==3,
    ...
    cc = X_kk(:,id);
    [ct, ax, V, id] = optimal_ellipsoid(X_kk,cc,ax0(:,randi(count))*(1+randn/10),[],100);
    nid = ~id;
...
elseif m==2,
    ...
    id = round(rand*count*N);
    cc = X_kk(:,id);
    [ct, ax, V, id] = optimal_ellipsoid(X_kk, cc, ax0(:,randi(count))*(1+randn/10),[],50);
    nid = ~id;
