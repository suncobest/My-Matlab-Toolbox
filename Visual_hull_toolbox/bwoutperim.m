function pout = bwoutperim(varargin)
%BWOUTPERIM Find perimeter of objects in binary image. 
%   BW2 = BWOUTPERIM(BW1) returns a binary image containing only the perimeter
%   pixels of objects in the input image BW1. A pixel is part of the outer
%   perimeter if it is zero and it is connected to at least one nonzero-valued
...
%       BW2 = bwoutperim(BW1,8);
%       figure, imshow(BW1)
%       figure, imshow(BW2)
%
%   See also BWAREA, BWPERIM, BWEULER, BWTRACEBOUNDARY, CONNDEF, IMFILL.
...
if (num_dims == 2) && isequal(conn, [0 1 0; 1 1 1; 0 1 0])
    ...
    p = bwmorph(b_dilated,'perim4');
    
elseif (num_dims == 2) && isequal(conn, ones(3,3))
    b_dilated = imdilate(b,conn);
    p = bwmorph(b_dilated,'perim8');
