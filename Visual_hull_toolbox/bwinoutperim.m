function pout = bwinoutperim(varargin)
%BWINOUTPERIM Find perimeter of objects in binary image.
%   BW2 = BWINOUTPERIM(BW1) returns a binary image containing only the perimeter
%   pixels of objects in the input image BW1. A pixel is part of the perimeter 
%   if it nonzero (zero) and it is connected to at least one zero-valued (nonzero-valued)
...
%
%   BW2 = BWOUTPERIM(BW1,CONN) specifies the desired connectivity.  CONN may
...
%       BW2 = bwinoutperim(BW1,8);
%       figure, imshow(BW1)
%       figure, imshow(BW2)
%
%   See also BWPERIM, BWAREA, BWBOUNDARIES, BWEULER, BWTRACEBOUNDARY, CONNDEF, IMFILL.
...
% bwmorph --- it works without padding the input.
    ...
    b_dilated = imdilate(b,conn);
    b_eroded = imerode(b,conn);
    p = b_dilated & ~b_eroded;
    idx = cell(1,num_dims);
    for k = 1 : num_dims
        idx{k} = 2:(size(p,k) - 1);
    end
