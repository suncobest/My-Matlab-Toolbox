%  See also extract_foreground1.
% This script extract foreground of image sequence using anisotropic diffusion to denoise images.
...
while n_frame == 0,
    [imgbase,imgdir]= uigetfile({'*.jpg;*.bmp;*.png;*.gif;*.tif','Image Files (*.jpg,*.bmp,*.png,*.gif,*.tif)';...
        ...
        '*.tif','TIFF Files(*.tif)'; '*.*','All Files(*.*)'},'Select the first frame image!');    % ѡ��ͼƬ·��
    if imgbase == 0,
        n_frame = 0;
...
%%% refine foreground by means of anisotropic diffusion
Not_save = 1;
while Not_save,
...
for kk = ind_active,
    ...
    for i=1:n,
        ...
        fore_kk(ibox(1,i) : ibox(3,i), ibox(2,i) : ibox(4,i)) = BW;
    end;
    % show the effect of sampled frames
%     if rem(kk,step)==1,
%         figure(2);
...
%         pause(wt);           % wait for wt second
%     end;
    imwrite(fore_kk,[imPreNumf strnum_frame{kk} '.png'],'png');   % background��Ϊ0
end;
fprintf(1,'\nImage initial foreground all saved!\n');

% save the variable settings for extration
string_save = ['save ' save_name ' bgprefix bgfmt fgprefix amplifier bw_thresh patch_sw ' ...
'del_region interest_box kappa num_iter privil_sw dblob_sw'];
eval(string_save);
